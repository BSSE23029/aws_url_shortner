...
class ApiClient {
  ...
  Future<ApiResponse<Map<String, dynamic>>> forgotPassword(String email) async {
    if (AppConfig.isDebugMode)
      return ApiResponse.success({}, message: 'Mock email sent');
  ...
  // ===========================================================================
  // API REQUESTS (THE CRITICAL PART)
  // ===========================================================================
  ...
  Future<ApiResponse<T>> request<T>({
    ...
    final fullUrl = '${AppConfig.currentLambdaUrl}$endpoint';
    _log("🚀 REQUEST: $method $fullUrl");

    if (requiresAuth && _authToken == null) {
      _log("❌ ERROR: Auth token missing but required.");
    }
    ...
    try {
      ...
      _log("⬅️ RESPONSE [${response.statusCode}]");
      _log("📄 RAW BODY: ${response.body}");

      return _handleResponse<T>(response);
    } catch (e, stack) {
      _log("❌ NETWORK EXCEPTION: $e");
      print(stack);
  ...
  ApiResponse<T> _handleResponse<T>(http.Response response) {
    if (response.statusCode >= 200 && response.statusCode < 300) {
      try {
        if (response.body.isEmpty) return ApiResponse.success(null);

        final data = jsonDecode(response.body);

        // Handling nested 'data' envelope if Lambda wraps it
        // Check 1: Is there a top-level 'data' key?
        ...
        // Check 2: Direct return?
        return ApiResponse.success(data as T?, message: 'Success');
      } catch (e) {
  ...
  ApiResponse<T> _handleCognitoError<T>(CognitoClientException e) {
    return ApiResponse.error(
      ...
      errorType: ApiErrorType.authenticationFailed,
    );
  }
  ...
  Future<ApiResponse<Map<String, dynamic>>> getDashboardSync() =>
      request(endpoint: AppConfig.dashboardSyncEndpoint, method: 'GET');
  ...
  Future<ApiResponse<Map<String, dynamic>>> deleteUrl(String id) => request(
    endpoint: '${AppConfig.urlsDeleteEndpoint}/$id',
    method: 'DELETE',
  );

  // Mocks...
  dynamic _mockSignIn(String email) => ApiResponse.success({
    'token': 'mock',
    'user': {'email': email},
  }, message: 'Mock Login');
  dynamic _mockSignUp(String email) => ApiResponse.success({
    'user': {'email': email},
    'confirmationRequired': true,
  });
  dynamic _mockMfa() =>
      ApiResponse.success({'token': 'mock'}, message: 'Mock MFA');
